...
#            if i != k:
#                data_path = self.config.rating_cv_path + self.config.dataset_name + "-" + str(i) + ".txt"
...
#    def testSet(self):
#        k = self.k_current
#        data_path = self.config.rating_cv_path + self.config.dataset_name + "-" + str(k) + ".txt"
#        if not os.path.isfile(data_path):
#            print("the format of ratings data is wrong!")
#            sys.exit()
#        with open(data_path, 'r') as f:
...
#                yield (int(float(u)), int(float(i)), float(r))
    ...
    def trainSet(self):
         ...
         df_save = pd.read_csv('E:\\testdata\\Epin2\\ratings2.txt', header=None)
         ...
         for i in range(len(save)):
            ...
            if  rand_num < self.config.size:
              u,i,r= int(save[i][0]), int(save[i][1]), save[i][2]
 ...
 #                    yield (int(u),int(i),float(r))
    ...
    def get_cold_start_users(self):
        ...
        #print('train users count', len(self.trainSet_i))
        print('test users count', len(self.testSet_u))       
